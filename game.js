...
const state={player:null,shadow:null,shadowLife:0,turn:0,selecting:false,cursor:null,cleared:false};
...
function checkGoal(){if(state.player.x===goal.x&&state.player.y===goal.y){state.cleared=true;state.selecting=false;state.cursor=null;message.textContent="Stage 1 클리어! 벽은 생성 경로가 아니라 목적지만 막습니다.";document.body.classList.add("cleared")}}
function move(dx,dy){if(state.cleared)return;state.selecting=false;state.cursor=null;const x=state.player.x+dx,y=state.player.y+dy;if(blocked(x,y)){message.textContent="벽이나 장애물로는 이동할 수 없습니다.";render();return}if(state.shadow&&x===state.shadow.x&&y===state.shadow.y){message.textContent="그림자가 있는 칸입니다. 스왑 버튼을 사용하세요.";render();return}state.player={x,y};message.textContent="한 칸 이동했습니다.";spendTurn()}
function createShadow(x,y){if(blocked(x,y)||state.shadow)return;if(!candidates().some(p=>p.x===x&&p.y===y))return;state.shadow={x,y};state.shadowLife=6;state.selecting=false;state.cursor=null;message.textContent="그림자를 만들었습니다. 생성 직후 남은 지속시간은 6턴입니다.";spendTurn(true)}
function swap(){if(!state.shadow||state.cleared)return;state.selecting=false;state.cursor=null;const p=state.player;state.player=state.shadow;state.shadow=p;message.textContent="본체와 그림자의 위치를 바꿨습니다.";spendTurn()}
function render(){board.innerHTML="";const valid=new Set(state.selecting&&!state.shadow?candidates().filter(p=>!blocked(p.x,p.y)).map(p=>key(p.x,p.y)):[]);const ring=new Set(state.selecting&&!state.shadow?candidates().map(p=>key(p.x,p.y)):[]);MAP.forEach((row,y)=>[...row].forEach((_,x)=>{const el=document.createElement("button");el.className="cell";el.setAttribute("role","gridcell");el.setAttribute("aria-label",x+", "+y);const k=key(x,y);if(walls.has(k))el.classList.add("wall");else if(obstacles.has(k))el.classList.add("obstacle");if(goal.x===x&&goal.y===y)el.classList.add("goal");if(valid.has(k))el.classList.add("candidate");else if(ring.has(k))el.classList.add("invalid");if(state.shadow&&state.shadow.x===x&&state.shadow.y===y)el.classList.add("shadow");if(state.cursor&&state.cursor.x===x&&state.cursor.y===y)el.classList.add("target-cursor");if(state.player.x===x&&state.player.y===y)el.classList.add("player");el.disabled=!valid.has(k);if(valid.has(k))el.addEventListener("click",()=>createShadow(x,y));board.append(el)}));turnCount.textContent=state.turn;shadowTurns.textContent=state.shadow?state.shadowLife+"턴":"없음";shadowButton.disabled=!!state.shadow||state.cleared;shadowButton.classList.toggle("active",state.selecting);shadowButton.textContent=state.selecting?"보라색 칸을 선택하세요":"그림자 생성 위치 선택";swapButton.disabled=!state.shadow||state.cleared}
function toggleShadowSelection(){
  if(state.shadow||state.cleared)return;
  state.selecting=!state.selecting;
  state.cursor=state.selecting?{...state.player}:null;
  message.textContent=state.selecting?"방향키로 빈 네모를 옮긴 뒤 Enter를 누르세요.":"그림자 선택을 취소했습니다.";
  render();
}
...
function confirmShadow(){
  ...
  const exactDistance=candidates().some(p=>p.x===x&&p.y===y);
  if(!exactDistance){message.textContent="그림자는 체비쇼프 거리 정확히 2인 칸에만 생성할 수 있습니다.";render();return}
...
document.addEventListener("keydown",e=>{
  ...
  if(d){
    e.preventDefault();
    if(state.selecting)moveCursor(...d);else move(...d);
  }
});
document.querySelector("#resetButton").addEventListener("click",()=>{Object.assign(state,{player:{...start},shadow:null,shadowLife:0,turn:0,selecting:false,cursor:null,cleared:false});document.body.classList.remove("cleared");message.textContent="걸어서는 나갈 수 없습니다. 그림자를 만들어 보세요.";render()});
render();